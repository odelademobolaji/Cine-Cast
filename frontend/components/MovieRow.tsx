 ...
 *
 * Demonstrates the leaf-server-component pattern from PDF section 1.4:
 * this fetches and renders entirely on the server. Each <Card> is also
 * a server component (no JS). Only the player route ships JS.
 */
...
export async function MovieRow({
  title,
  source,
  mediaType = 'movie',
}: {
  ...
  return (
    <section className="row">
      ...
      <div className="row-grid">
        {items.map((m) => (
          <Card key={m.id} movie={m} mediaType={mediaType} />
        ))}
      </div>
    </section>
  );
